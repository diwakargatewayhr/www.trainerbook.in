...
$manage_banner = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `banner` WHERE `id` = '" . @$_REQUEST['edit'] . "'"));
...
if (isset($_REQUEST['submit']) && $_REQUEST['edit'] == '') {
    ...
    $query = "INSERT INTO `banner` (`id`, `banner_image`,`created_at`) 
              VALUES ('', $ImageFileNameSQL,now())";

    mysqli_query($CONN, $query);

    header("location:manage_banner?created=yes");
    exit;
}
...
if (isset($_REQUEST['submit']) && $_REQUEST['edit'] != '') {
    ...
    $updateQuery = "UPDATE `banner` SET `banner_image` = $imageFileNameSQL WHERE `id` = '$pid'";
    mysqli_query($CONN, $updateQuery);

    header("location: manage_banner?updated=yes");
    exit;
}

if (isset($_REQUEST['DelId']) != '') {
    mysqli_query($CONN, "DELETE FROM `banner` WHERE `id` = '" . $_REQUEST['DelId'] . "' ");
    header("location:manage_banner");
    exit;
}
?>
<!DOCTYPE html>
                ...
                <div class="mw1000 center-block" id="hidden_div" <?php if (@$_REQUEST['edit'] == '') { ?> style="display:none;" <?php } ?>>
                    ...
                    <div class="panel mb35">
                        <div class="panel-heading">
                            <span class="panel-title">Add Banner</span>
                        </div>

                        <?php if (@$_REQUEST['created'] == 'yes') { ?>
                            <div class="alert alert-success dark alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                                <i class="fa fa-info pr10"></i> Congrats! Banner added successfully!
                            </div>
                        <?php } ?>
                        <?php if (@$_REQUEST['updated'] == 'yes') { ?>
                            <div class="alert alert-success dark alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                                <i class="fa fa-info pr10"></i> Congrats! Banner updated successfully!
                            </div>
                        <?php } ?>
                                ...
                                <form method="post" enctype="multipart/form-data">
                                    <div class="col-md-6">
                                        <div class="section row mb25">
                                            <label for="image" class="field-label col-sm-4 ph10">Banner Image</label>
                                            <div class="col-sm-8 ph10">
                                                <input type="file" name="image" class="gui-input" <?php if (empty($manage_banner['banner_image'])) {
                                                                                                        echo 'required="required"';
                                                                                                    } ?>>
                                                <?php if (@$manage_banner['banner_image'] != '') { ?>
                                                    <img src="../upload/profile/<?php echo @$manage_banner['banner_image']; ?>" height="100px" alt="" />
                                                <?php } ?>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="chute chute-center" style="padding-top:0px;">
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="panel">
                                <div class="panel-heading">
                                    <span class="panel-title hidden-xs">All Banner</span>
                                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                                <?php
                                                $sl = 0;
                                                $result2 = mysqli_query($CONN, "SELECT * FROM `banner` ORDER BY `id` DESC");
                                                while ($getValue = mysqli_fetch_array($result2)) {
                                                    $sl++;
                                                ?>
                                                    <tr>
                                                        <td><?php echo $sl; ?></td>
                                                        <td><img src="../upload/profile/<?php echo @$getValue['banner_image']; ?>" alt="Image" style="width: 100px; height: auto;"></td>
                                                        <td class="text-right">
                                                            <div class="btn-group text-right">
                                                                ...
                                                                <ul class="dropdown-menu" role="menu">
                                                                    <li><a href="manage_banner?edit=<?php echo $getValue['id']; ?>">Edit</a></li>
                                                                    <li><a href="manage_banner?DelId=<?php echo $getValue['id']; ?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a></li>
                                                                </ul>
                                                            </div>
                                                        </td>
                                                    </tr>
