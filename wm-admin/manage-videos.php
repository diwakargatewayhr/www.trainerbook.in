...
$manage_documents = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `trainer_video` WHERE `id` = '" . @$_REQUEST['edit'] . "' AND `fk_user_id` = '" . @$trainer_id . "' "));
// echo "<pre>";
...
if (isset($_REQUEST['submit']) && $_REQUEST['edit'] == '') {
    ...
    if (!empty($_FILES['video']['name'])) {
        $video_target_path = "../upload/profile/" . $video_random . basename($_FILES['video']['name']);
    ...
    // mysqli_query($CONN, "INSERT INTO `trainer_video` (`title`, `description`, `image`, `fk_user_id`, `status`) VALUES ('" . $_REQUEST['doc_title'] . "', '" . $_REQUEST['doc_description'] . "', '" . $ImageFileNameSQL . "', '" . $trainer_id . "', 'Y')");

    $query = "INSERT INTO `trainer_video` (`title`, `description`, `video`, `fk_user_id`, `status`) 
              VALUES ('$title',  '$description',$VideoFileNameSQL,'$trainer_id' ,'Y')";

    mysqli_query($CONN, $query);
    header("location:manage-videos?created=yes&trainer=$trainer_id");
    exit;
}
if (isset($_REQUEST['submit']) && $_REQUEST['edit'] != '') {
    $pid = $_REQUEST['edit'];

    $existingDataQuery = "SELECT `video` FROM `trainer_video` WHERE `id` = '$pid'";
    ...
    mysqli_query($CONN, "UPDATE trainer_video SET 
        `title` = '" . $title . "', 
    ...
    header("location:manage-videos?updated=yes&trainer=$trainer_id");
    exit;
}


if (isset($_REQUEST['DelId']) != '') {
    mysqli_query($CONN, "DELETE FROM `trainer_video` WHERE `id` = '" . $_REQUEST['DelId'] . "' ");
    header("location:manage-videos?trainer=$trainer_id");
    exit;
}
?>
<!DOCTYPE html>
                        ...
                        <div class="panel-body br-t">
                            <div class="allcp-form theme-primary">
                                <form method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="col-md-6">
                                        <div class="section row mb25">
                                            <label for="video" class="field-label col-sm-4 ph10">Video</label>
                                            <div class="col-sm-8 ph10">
                                                <input type="file" name="video" class="gui-input">
                                                <?php if (@$manage_documents['video'] != '') { ?>
                                                    <video controls style="width: 228px; height: 150px;">
                                                        <source src="../upload/profile/<?php echo @$manage_documents['video']; ?>" type="video/mp4">
                                    ...
                                    <div class="table-responsive">
                                        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                                <?php
                                                $sl = 0;
                                                $result2 = mysqli_query($CONN, "SELECT * FROM `trainer_video` ORDER BY `id` DESC");
                                                ...
                                                ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?php if (!empty($getValue['video']) && file_exists('../upload/profile/' . $getValue['video'])) : ?>
                                                                <video width="320" height="240" controls>
                                                                    <source src="../upload/profile/<?php echo $getValue['video']; ?>" type="video/mp4">
                                                                    <source src="../upload/profile/<?php echo str_replace('.mp4', '.webm', $getValue['video']); ?>" type="video/webm">
                                                                    Your browser does not support the video tag.
                                                                </video>
                                                            <?php else : ?>
                                                                No video found
                                                        ...
                                                        <td class="text-right">
                                                            <div class="btn-group text-right">
                                                                ...
                                                                <ul class="dropdown-menu" role="menu">
                                                                    <li><a href="manage-videos?edit=<?php echo $getValue['id']; ?>&trainer=<?php echo $trainer_id ?>">Edit</a></li>
                                                                    <li><a href="manage-videos?DelId=<?php echo $getValue['id']; ?>&trainer=<?php echo $trainer_id ?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a></li>
                                                                </ul>
                                                            </div>
                                                        </td>
                                                    </tr>
