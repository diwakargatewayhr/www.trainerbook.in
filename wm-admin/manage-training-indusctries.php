...
$training_industries = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `training_industries` WHERE `id` = '" . @$_REQUEST['edit'] . "'"));

if (isset($_REQUEST['submit']) && $_REQUEST['industries_name'] != '' && $_REQUEST['edit'] != '') {


    mysqli_query($CONN, "UPDATE `training_industries` SET `industries_name` = '" . $_REQUEST['industries_name'] . "' WHERE id = '" . $_REQUEST['edit'] . "'");
    header("location:manage-training-indusctries?&updated=yes");
    exit;
}

if (isset($_REQUEST['submit']) && $_REQUEST['industries_name'] != '' && @$_REQUEST['edit'] == '') {

    mysqli_query($CONN, "INSERT INTO `training_industries` (`industries_name`) VALUES ('" . $_REQUEST['industries_name'] . "')");
    header("location:manage-training-indusctries?created=yes");
    exit;
}

if (isset($_REQUEST['DelId']) != '') {
    mysqli_query($CONN, "DELETE FROM `training_industries` WHERE `id` = '" . $_REQUEST['DelId'] . "' ");
    header("location:manage-training-indusctries");
    exit;
}
?>
<!DOCTYPE html>
                ...
                <div class="mw1000 center-block" id="hidden_div" <?php if (@$_REQUEST['edit'] == '') { ?> style="display:none;" <?php } ?>>
                    ...
                    <div class="panel mb35">
                        <div class="panel-heading">
                            <span class="panel-title">Add Training Type</span>
                        </div>

                        <?php if (@$_REQUEST['created'] == 'yes') { ?>
                            <div class="alert alert-success dark alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                                <i class="fa fa-info pr10"></i> Congrats! Training Industries added successfully!
                            </div>
                        <?php } ?>
                        <?php if (@$_REQUEST['updated'] == 'yes') { ?>
                            <div class="alert alert-success dark alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                                <i class="fa fa-info pr10"></i> Congrats! Training Industries updated successfully!
                            </div>
                        <?php } ?>
                                ...
                                <form method="post" enctype="multipart/form-data">
                                    <div class="col-md-6">
                                        <div class="section row mb25">
                                            <label for="video" class="field-label col-sm-4 ph10">Training Industries</label>
                                            <div class="col-sm-8 ph10">
                                                <input type="text" name="industries_name" class="gui-input" value="<?php echo @$training_industries['industries_name'] ?>">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                ...
                <div class="chute chute-center" style="padding-top:0px;">
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="panel">
                                <div class="panel-heading">
                                    <span class="panel-title hidden-xs">All Training Industries</span>
                                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                            <thead>
                                                <tr class="bg-light">
                                                    <th class="">Sl. No.</th>
                                                    <th class="">Training Industries</th>
                                            ...
                                            <tbody>
                                                <?php
                                                $sl = 0;
                                                $result2 = mysqli_query($CONN, "SELECT * FROM `training_industries` ORDER BY `id` DESC");
                                                while ($getValue = mysqli_fetch_array($result2)) {
                                                    $sl++;
                                                ?>
                                                    <tr>
                                                        <td><?php echo $sl; ?></td>
                                                        <td><?php echo @$getValue['industries_name']; ?></td>
                                                        <td class="text-right">
                                                            <div class="btn-group text-right">
                                                                ...
                                                                <ul class="dropdown-menu" role="menu">
                                                                    <li><a href="manage-training-indusctries?edit=<?php echo $getValue['id']; ?>">Edit</a></li>
                                                                    <li><a href="manage-training-indusctries?DelId=<?php echo $getValue['id']; ?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a></li>
                                                                </ul>
                                                            </div>
                                                        </td>
                                                    </tr>
