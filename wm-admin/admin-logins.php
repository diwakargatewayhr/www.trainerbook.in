...
$sql = mysqli_query($CONN,"SELECT * FROM `admin_lastlogin`");
...
if (isset($_POST['reset_history'])) {
   $sql2 = mysqli_query($CONN,"DELETE FROM `admin_lastlogin` WHERE id NOT IN ( SELECT t.id FROM (SELECT id FROM admin_lastlogin ORDER BY id DESC LIMIT 3 ) t)");
}
                ...
                <div class="mw1000 center-block">
                    ...
                    <?php echo $msg; ?>
                            <div class="panel-heading">
                                <div class="panel-title hidden-xs">Admin Last Logins</div>
                            </div>
                            <div class="panel-body pn">
                                <div class="table-responsive">
                                    <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        <thead>
                                        <tr>
                                            <th class="va-m">Date Time</th>
                                            <th class="va-m">Users</th>
                                       ...
                                       <tbody>
                                        <?php 
                                         $getVisitor = mysqli_query($CONN,"SELECT * FROM `admin_lastlogin` ORDER by id DESC LIMIT 0, 50");
                                            while($trackUser = mysqli_fetch_array($getVisitor)) { 
                                        ?>
                                        <tr>
                                            <td><?php echo $trackUser['timestamp']; ?></td>
                                            <td><?php $getUsername = mysqli_fetch_array(mysqli_query($CONN,"SELECT * FROM `admin_login` WHERE id = '".$trackUser['adminid']."'")); echo $getUsername['firstname']; ?></td>
