...
$category = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `testimonial` WHERE `id` = '".@$_REQUEST['edit']."'"));

if(isset($_REQUEST['submit']) && $_REQUEST['client_name'] != '' && $_REQUEST['edit'] == '')
{
	$random = rand(1,999999);
	$target_path = "../upload/testimonial/".$random;
	$target_path = $target_path . basename($_FILES['document']['name']);
	$FileName = $random.$_FILES['document']['name'];
	...
	mysqli_query($CONN, "INSERT INTO `testimonial` (`id`, `client_name`, `designation`, `profile_pic`, `details`, `status`) VALUES ('', '".$_REQUEST['client_name']."', '".$_REQUEST['designation']."', '$FileName', '".$_REQUEST['details']."', '".$_REQUEST['status']."')");
	header("location:testimonials?created=yes"); exit;
}
...
if(isset($_REQUEST['submit']) && $_REQUEST['client_name'] != '' && $_REQUEST['edit'] != '')
{
	...
	mysqli_query($CONN, "UPDATE `testimonial` SET `client_name` = '".$_REQUEST['client_name']."', `designation` = '".$_REQUEST['designation']."', `profile_pic` = '$FileName', `details` = '".$_REQUEST['details']."', `status` = '".$_REQUEST['status']."' WHERE `id` = '".$_REQUEST['edit']."'");
	header("location:testimonials?edit=".$_REQUEST['edit']."&updated=yes"); exit;
}

if(isset($_REQUEST['DelId'])!='')
{
    mysqli_query($CONN, "DELETE FROM `testimonial` WHERE `id` = '".$_REQUEST['DelId']."' ");
            ...
            <div class="mw1000 center-block" id="hidden_div" <?php if(@$_REQUEST['edit']=='') { ?> style="display:none;" <?php } ?> >
                    ...
                    <div class="panel mb35">
                        <div class="panel-heading">
                            <span class="panel-title">Add Testimonial</span>
                        </div>
                        
                        <?php if(@$_REQUEST['created'] == 'yes') { ?>
							<div class="alert alert-success dark alert-dismissable">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
								<i class="fa fa-info pr10"></i> Congrats! Testimonial added successfully!
							</div>
						<?php } ?>
						<?php if(@$_REQUEST['updated'] == 'yes') { ?>
							<div class="alert alert-success dark alert-dismissable">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
								<i class="fa fa-info pr10"></i> Congrats! Testimonial updated successfully!
							</div>
						<?php } ?>
                        ...
                        <div class="panel-body br-t">
                            ...
                            <label for="refund-policy" class="field-label col-sm-4 ph10">Client Name</label>
                                <div class="col-sm-8 ph10">
                                    <input type="text" name="client_name" class="gui-input" value="<?php echo @$category['client_name']; ?>">
                                </div>
                            </div></div>
                            ...
                            <label for="refund-policy" class="field-label col-sm-4 ph10">Company</label>
                                <div class="col-sm-8 ph10">
                                    <input type="text" name="designation" class="gui-input" value="<?php echo @$category['designation']; ?>">
                                </div>
                            </div></div>
                            ...
                            <label for="refund-policy" class="field-label col-sm-4 ph10">Profile Picture</label>
                                <div class="col-sm-8 ph10">
                                    <input type="file" name="document" class="gui-input">
                                    <img src="../upload/testimonial/<?php echo @$category['profile_pic']; ?>" width="100px;" alt="" />
                                </div>
                            </div></div>
                    ...
                    <input type="submit" name="submit" class="btn btn-bordered btn-primary pull-right" value="<?php if(@$_REQUEST['edit']!='') { ?>UPDATE<?php } else { ?>ADD<?php } ?> Testimonial">
                </div>
            ...
            <div class="chute chute-center">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="panel">
                            <div class="panel-heading">
                                <span class="panel-title hidden-xs"> Testimonials</span>
                            </div>
                            <div class="panel-body pn">
                                <div class="table-responsive">
                                    ...
                                    <?php
                                        $sl = 0;
                                        $result2 = mysqli_query($CONN,"SELECT * FROM `testimonial` ORDER BY `id` DESC");
                                        while($getValue = mysqli_fetch_array($result2)) { 
                                        $sl++;
                                    ?>
                                        <tr>
                                            <td><?php echo $sl; ?></td>
                                            <td class="" style="text-align:left;"><?php echo $getValue['client_name']; ?></td>
											<td class="" style="text-align:left;"><?php echo $getValue['designation']; ?></td>
                                            <td class="" style="text-align:left;"><img src="../upload/testimonial/<?php echo $getValue['profile_pic']; ?>" width="100px;" alt="" /></td>
                                            <td><?php echo $getValue['details']; ?></td>
                                            <td class="text-right">
                                                <div class="btn-group text-right">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        <li><a href="testimonials?edit=<?php echo $getValue['id'];?>">Edit</a></li>
                                                        <li><a href="testimonials?DelId=<?php echo $getValue['id']; ?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a></li>
                                                    </ul>
                                                </div>
                                            </td>
                                        </tr>
