...
{
    mysqli_query($CONN, "DELETE FROM `business_leads` WHERE `id` = '".$_REQUEST['DelId']."' ");
    header("location:manage_leads?id=".$_REQUEST['id']); exit;
}

if($_REQUEST['status'] != '' && $_REQUEST['id'] != '')
{
	mysqli_query($CONN, "UPDATE `business_leads` SET `status` = '".$_REQUEST['status']."' WHERE `id` = '".$_REQUEST['id']."' ");
    header("location:manage_leads?id=".$_REQUEST['id']); exit;
}
?>
<!DOCTYPE html>
                    ...
                    <div class="col-xs-12">
                        <div class="panel">
                            <div class="panel-heading">
                                <span class="panel-title hidden-xs">All Subscriptions</span>
                            </div>
                            <div class="panel-body pn">
                                <div class="table-responsive">
                                    ...
                                    <?php
                                        $sl = 0;
                                        $result2 = mysqli_query($CONN,"SELECT * FROM `leads`");
                                    ...
                                    ?>
                                        <tr>
											...
											<td class=""><?=$getValue['months']?> Months - ₹<?=$getValue['price']?></td>
											<td class=""><?php echo $getValue['datetime']; ?></td>
											<td class=""><button type="button" class="btn btn-success br2 btn-xs fs12">New</button></td>
                                            <td class="text-right">
                                                <div class="btn-group text-right">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        <li>
                                                            <a href="manage_leads?DelId=<?php echo $_REQUEST['id'];?>&DelId=<?php echo $getValue['id']; ?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a>
                                                        </li>                                                        
                                                    </ul>
                                                </div>
                                            </td>
