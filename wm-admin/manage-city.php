...
$category = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `city` WHERE `id` = '".@$_REQUEST['edit']."'"));
if(isset($_REQUEST['submit']) && $_REQUEST['city'] != '' && @$_REQUEST['edit'] == '')
{
	
	mysqli_query($CONN, "INSERT INTO `city` (`city`, `status`) VALUES ('".$_REQUEST['city']."', '".$_REQUEST['status']."')");
	header("location:manage-city?created=yes"); exit;
}
...
if(isset($_REQUEST['submit']) && $_REQUEST['city'] != '' && $_REQUEST['edit'] != '')
{
	...
	mysqli_query($CONN, "UPDATE `city` SET `city` = '".$_REQUEST['city']."', `status` = '".$_REQUEST['status']."' WHERE id = '".$_REQUEST['edit']."'");
	header("location:manage-city?edit=".$_REQUEST['edit']."&updated=yes"); exit;
}

if(isset($_REQUEST['DelId'])!='')
{
    mysqli_query($CONN, "DELETE FROM `city` WHERE `id` = '".$_REQUEST['DelId']."' ");
    header("location:manage-city"); exit;
}
?>
<!DOCTYPE html>
<html>
...
<div id="main">
    ...
    <!--  Main Wrapper -->
        ...
        <!-- Topbar -->
        <header id="topbar" class="ph10" style="padding-top:75px;">
        ...
        <form action="" method="post" enctype="multipart/form-data" name="form1">
            ...
            <div class="mw1000 center-block" id="hidden_div" <?php if(@$_REQUEST['edit']=='') { ?> style="display:none;" <?php } ?> >
                    ...
                    <div class="panel mb35">
                        ...
                        <?php if(@$_REQUEST['created'] == 'yes') { ?>
							<div class="alert alert-success dark alert-dismissable">
								...
								<i class="fa fa-info pr10"></i> Congrats! City added successfully!
							</div>
						<?php } ?>
						<?php if(@$_REQUEST['updated'] == 'yes') { ?>
							<div class="alert alert-success dark alert-dismissable">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
								<i class="fa fa-info pr10"></i> Congrats! City updated successfully!
							</div>
						<?php } ?>
                        ...
                        <div class="panel-body br-t">
                            ...
                            <label for="refund-policy" class="field-label col-sm-4 ph10">City Name</label>
                                <div class="col-sm-8 ph10">
                                    <input type="text" name="city" class="gui-input" value="<?php echo @$category['city']; ?>">
                                </div>
                            </div>
                    ...
                    <input type="submit" name="submit" class="btn btn-bordered btn-primary pull-right" value="<?php if(@$_REQUEST['edit']!='') { ?>UPDATE<?php } else { ?>ADD<?php } ?> City">
                </div>
        ...
        <section id="content" class="table-layout animated fadeIn">
            ...
            <!--  Column Center  -->
            <div class="chute chute-center" style="padding: 0px 32px 50px;">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="panel">
                            <div class="panel-heading">
                                <span class="panel-title hidden-xs">Manage City</span>
                            </div>
                            <div class="panel-body pn">
                                <div class="table-responsive">
                                    ...
                                    <?php
                                        $sl = 0;
                                        $result2 = mysqli_query($CONN,"SELECT * FROM `city` ORDER BY `id` DESC");
                                        while($getValue = mysqli_fetch_array($result2)) { 
                                        $sl++;
                                    ?>
                                        <tr>
                                            <td><?php echo $sl; ?></td>
                                            <td class="" style="text-align:left;"><?php echo $getValue['city']; ?></td>
                                            ...
                                            <td class="text-right">
                                                <div class="btn-group text-right">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        <li>
                                                            <a href="manage-city?edit=<?php echo $getValue['id'];?>">Edit</a>
                                                        </li>
                                                        <li>
                                                            <a href="manage-city?DelId=<?php echo $getValue['id']; ?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a>
                                                        </li>                                                        
                                                    </ul>
                                                </div>
                                            </td>
