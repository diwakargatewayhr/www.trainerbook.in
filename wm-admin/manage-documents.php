...
$manage_documents = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `trainer_material` WHERE `id` = '" . @$_REQUEST['edit'] . "' AND `fk_user_id` = '" . @$trainer_id . "' "));
// echo "<pre>";
// print_r($manage_documents);
// exit;
if (isset($_REQUEST['submit']) && $_REQUEST['edit'] == '') {
    ...
    if (!empty($_FILES['image']['name'])) {
        $image_target_path = "../upload/profile/" . $image_random . basename($_FILES['image']['name']);
    ...
    $title = $_REQUEST['doc_title'];
    $description = $_REQUEST['doc_description'];
    // mysqli_query($CONN, "INSERT INTO `trainer_material` (`title`, `description`, `image`, `fk_user_id`, `status`) VALUES ('" . $_REQUEST['doc_title'] . "', '" . $_REQUEST['doc_description'] . "', '" . $ImageFileNameSQL . "', '" . $trainer_id . "', 'Y')");

    $query = "INSERT INTO `trainer_material` (`title`, `description`, `image`, `fk_user_id`, `status`) 
              VALUES ('$title','$description',$ImageFileNameSQL,'$trainer_id' ,'Y')";

    mysqli_query($CONN, $query);
    header("location:manage-documents?created=yes&trainer=$trainer_id");
    exit;
}
if (isset($_REQUEST['submit']) && $_REQUEST['edit'] != '') {
    $pid = $_REQUEST['edit'];

    $existingDataQuery = "SELECT `image` FROM `trainer_material` WHERE `id` = '$pid'";
    ...
    $title = $_REQUEST['doc_title'];
    $description = $_REQUEST['doc_description'];
    mysqli_query($CONN, "UPDATE trainer_material SET 
        `title` = '" . $title . "', 
        `description` = '" . $description . "', 
        `image` = '" . $ImageFileNameSQL . "', 
        `fk_user_id` = '" . $trainer_id . "' 
    WHERE `id` = '" . $_REQUEST['edit'] . "'");

    header("location: manage-documents?updated=yes&trainer=$trainer_id");
    exit;
}


if (isset($_REQUEST['DelId']) != '') {
    mysqli_query($CONN, "DELETE FROM `trainer_material` WHERE `id` = '" . $_REQUEST['DelId'] . "' ");
    header("location:manage-documents?trainer=$trainer_id");
    exit;
}
?>
<!DOCTYPE html>
...
<body class="sales-stats-page">
    ...
    <div id="main">
        ...
        <section id="content_wrapper">
            ...
            <!-- Topbar -->
            <header id="topbar" class="ph10" style="padding-top:5rem;">
                ...
                <div class="mw1000 center-block" id="hidden_div" <?php if (@$_REQUEST['edit'] == '') { ?> style="display:none;" <?php } ?>>
                    ...
                    <div class="panel mb35">
                        <div class="panel-heading">
                            <span class="panel-title">Add Documents</span>
                        </div>

                        <?php if (@$_REQUEST['created'] == 'yes') { ?>
                            <div class="alert alert-success dark alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                                <i class="fa fa-info pr10"></i> Congrats! Documents added successfully!
                            </div>
                        <?php } ?>
                        <?php if (@$_REQUEST['updated'] == 'yes') { ?>
                            <div class="alert alert-success dark alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                                <i class="fa fa-info pr10"></i> Congrats! Documents updated successfully!
                            </div>
                        <?php } ?>
                                ...
                                <form method="post" enctype="multipart/form-data">
                                    <div class="col-md-6">
                                        <div class="section row mb25">
                                            <label for="training-type" class="field-label col-sm-4 ph10">Title</label>
                                            <div class="col-sm-8 ph10">
                                                <input type="text" name="doc_title" class="gui-input" value="<?php echo @$manage_documents['title'] ?>"></input>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="section row mb25">
                                            <label for="training-type" class="field-label col-sm-4 ph10">Description</label>
                                            <div class="col-sm-8 ph10">
                                                <input type="text" name="doc_description" class="gui-input" value="<?php echo @$manage_documents['description'] ?>"></input>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="section row mb25">
                                            <label for="image" class="field-label col-sm-4 ph10">Documents Image</label>
                                            <div class="col-sm-8 ph10">
                                                <input type="file" name="image" class="gui-input" <?php if (empty($manage_documents['image'])) {
                                                                                                        echo 'required="required"';
                                                                                                    } ?>>
                                                <?php if (@$manage_documents['image'] != '') { ?>
                                                    <img src="../upload/profile/<?php echo @$manage_documents['image']; ?>" height="6.25rem" alt="" />
                                                <?php } ?>
                                            </div>
                                        </div>
                                    </div>
            ...
            <section id="content" class="table-layout animated fadeIn">
                ...
                <!--  Column Center  -->
                <div class="chute chute-center" style="padding-top:0rem;">
                    <!-- Products Status Table -->
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="panel">
                                <div class="panel-heading">
                                    <span class="panel-title hidden-xs">Trainer Document</span>
                                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                                <?php
                                                $sl = 0;
                                                $result2 = mysqli_query($CONN, "SELECT * FROM `trainer_material` ORDER BY `id` DESC");
                                                while ($getValue = mysqli_fetch_array($result2)) {
                                                    $sl++;
                                                ?>
                                                    <tr>
                                                        <td><?php echo $sl; ?></td>
                                                        <td class="" data-title="title"><?php echo $getValue['title']; ?></td>
                                                        <td class="" data-title="description"><?php echo $getValue['description']; ?></td>
                                                        <td><img src="../upload/profile/<?php echo @$getValue['image']; ?>" alt="Image" style="width: 6.25rem; height: auto;"></td>
                                                        <td class="text-right">
                                                            <div class="btn-group text-right">
                                                                ...
                                                                <ul class="dropdown-menu" role="menu">
                                                                    <li><a href="manage-documents?edit=<?php echo $getValue['id']; ?>&trainer=<?php echo $trainer_id ?>">Edit</a></li>
                                                                    <li><a href="manage-documents?DelId=<?php echo $getValue['id']; ?>&trainer=<?php echo $trainer_id ?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a></li>
                                                                </ul>
                                                            </div>
                                                        </td>
                                                    </tr>
