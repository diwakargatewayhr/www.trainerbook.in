...
if(isset($_REQUEST['submit']) && $_REQUEST['fullname'] != '' && $_REQUEST['edit'] == '')
{
	...
	mysqli_query($CONN, "INSERT INTO `user_register` (`id`, `fullname`, `phone`, `email`, `aadhaar`, `pan_card`, `dateofbirth`, `gender`, `profile_pic`, `bank_name`, `account_holder`, `ifsc_code`, `bank_branch`, `account_number`, `date`, `status`, `datetime`) VALUES ('', '".$_REQUEST['fullname']."', '".$_REQUEST['phone']."', '".$_REQUEST['email']."', '".$_REQUEST['aadhaar']."', '".$_REQUEST['pan_card']."', '".$_REQUEST['dateofbirth']."', '".$_REQUEST['gender']."', '$FileName', '".$_REQUEST['bank_name']."', '".$_REQUEST['account_holder']."', '".$_REQUEST['ifsc_code']."', '".$_REQUEST['bank_branch']."', '".$_REQUEST['account_number']."', '".date('d M, Y')."', 'Y', now())");
	header("location:providers?created=yes"); exit;
}
...
if(@$_REQUEST['pid'] != '' && @$_REQUEST['status'] != '')
{
    ...
    header("location:providers"); exit;
}
...
if(isset($_REQUEST['DelId'])!='')
{
    ...
    header("location:earning_log"); exit;
}
?>
<!DOCTYPE html>
<html>
            ...
            <div class="mw1000 center-block" id="hidden_div" <?php if(@$_REQUEST['edit']=='') { ?> style="display:none;" <?php } ?> >
                    ...
                    <div class="panel mb35">
                        <div class="panel-heading">
                            <span class="panel-title">Add Professional</span>
                        </div>
                        
                        <?php if(@$_REQUEST['created'] == 'yes') { ?>
							<div class="alert alert-success dark alert-dismissable">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
								<i class="fa fa-info pr10"></i> Congrats! Professional added successfully!
							</div>
						<?php } ?>
						<?php if(@$_REQUEST['updated'] == 'yes') { ?>
							<div class="alert alert-success dark alert-dismissable">
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
								<i class="fa fa-info pr10"></i> Congrats! Professional updated successfully!
							</div>
						<?php } ?>
                        ...
                        <div class="panel-body br-t">
                            ...
                            <label for="refund-policy" class="field-label col-sm-4 ph10">Professional Name</label>
            ...
            <div class="chute chute-center">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="panel">
                            <div class="panel-heading">
                                <span class="panel-title hidden-xs">Earning Logs</span>
                            </div>
                            <div class="panel-body pn">
                                <div class="table-responsive">
                                    ...
                                    <?php
                                        ...
                                        if(@$_REQUEST['pid'] == '')
                                        {
                                            $result2 = mysqli_query($CONN, "SELECT * FROM `earnigs`");
                                        }
                                        else
                                        {
                                            $result2 = mysqli_query($CONN, "SELECT * FROM `earnigs` WHERE `professional` = '".$_REQUEST['pid']."'");
                                        }
                                        
                                        while($getValue = mysqli_fetch_array($result2)) { 
                                            $professional = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `user_register` WHERE `id` = '".$getValue['professional']."'"));
                                            $customer = mysqli_fetch_array(mysqli_query($CONN, "SELECT * FROM `user_register` WHERE `id` = '".$getValue['userid']."'"));
                                        $sl++;
                                    ?>
                                        <tr>
                                            <td><?php echo $getValue['orderid']; ?></td>
                                            <td style="text-align:left;"><?php echo @$customer['fullname']; ?> - <?php echo @$customer['phone']; ?></td>
                                            <td style="text-align:left;"><?php echo @$professional['fullname']; ?></td>
											<td><?php echo @$professional['phone']; ?></td>
                                            <td><?php $sumcart = mysqli_fetch_array(mysqli_query($CONN, "SELECT SUM(price) AS TotalPrice FROM `mycart` WHERE `transaction` = '".$getValue['orderid']."'")); ?>₹<?=$sumcart['TotalPrice']?></td>
											<td style="text-align:left;"><?php if($getValue['paid'] == 'Y') { ?><a href="#"><span class="label label-success">Paid</span></a><?php } ?><?php if($getValue['paid'] == 'N') { ?><a href="#"><span class="label label-danger">Unpaid</span></a><?php } ?></td>
											<td class="text-right">
                                                <div class="btn-group text-right">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        ...
                                                        <li><a href="earning_log?pid=<?=@$_REQUEST['pid']?>&DelId=<?php echo @$getValue['id']; ?>" onClick="return confirm('Due to System Security. You are not allowed to delete this log. Contact with webmaster for approval.')">Delete</a></li>
                                                    </ul>
                                                </div>
                                            </td>
                                        </tr>
